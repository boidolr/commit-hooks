...
def test_suggests_correct_message(input_s, expected, tmpdir, dictionary_path, capsys):
    ...
    path.write(input_s)
    assert main((path.strpath, '-d', dictionary_path)) == 1
...
def test_keeps_correct_message(input_s, tmpdir, dictionary_path, capsys):
    path = tmpdir.join('message')
    path.write(input_s)
    assert main((path.strpath, '-d', dictionary_path)) == 0
    corrected = capsys.readouterr().out
    assert '' == corrected
...
def test_print_only_fixed_line(input_s, expected, unexpected, tmpdir, dictionary_path, capsys):
    path = tmpdir.join('message')
    path.write(input_s)
    assert main((path.strpath, '-d', dictionary_path)) == 1
    corrected = capsys.readouterr().out
